...
var wire = new i2c(value.DEFAULT_I2C_ADDRESS, {device: limit.I2C_MAX_LEN});
...
function picoborgrev(){
    return {
        ...
        SetMotor2: function(power, callback){
            if ( power < 0 ){
                wire.writeBytes(command.SET_A_REV, power, function(err) {
                    callback(err);
                });
            } else {
                if ( power > PWM_MAX ){
                ...
                wire.writeBytes(command.SET_A_FWD, power, function(err) {
                    callback(err);
                });
            }
        },
        ...
        GetMotor2: function(callback){
            wire.readBytes(command.GET_A, limit.I2C_MAX_LEN, function(err, res) {
                if ( err ){
                    ...
                    power = parseFloat(res[2]) / parseFloat(limit.PWM_MAX);
                    if ( res[1] == value.FWD ){
                        callback(null, power);
        ...
        SetMotor1: function(power, callback){
              if ( power < 0 ){
                  wire.writeBytes(command.SET_B_REV, power, function(err) {
                      callback(err);
                  });
              } else {
                  if ( power > PWM_MAX ){
                  ...
                  wire.writeBytes(command.SET_B_FWD, power, function(err) {
                      callback(err);
                  });
              }  
        },
        ...
        // require byte
        SetNewAddress: function(newAddress){
